...
def add_red(psrs, amp=1e-13, gamma=3., rand=False):
    for psr in psrs:
        try:
            if rand==True: 
                amp = random.uniform(1e-15,1e-11)
                gamma = random.uniform(0,7)
            LT.add_red(psr,amp,gamma)
            psr.fit()
        except:
            pass
    return psrs
...
def add_dm(psrs, amp=1e-13, gamma=3., rand=False):
    for psr in psrs:
        try: 
            if rand==True: 
                amp = random.uniform(-15,-11)
                gamma = random.uniform(0,7)
